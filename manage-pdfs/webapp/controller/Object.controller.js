sap.ui.define([
	...
	return BaseController.extend("web.manage-pdfs.controller.Object", {
		...
		_onObjectMatched : function (oEvent) {
			...
			this._bindView("/Pdfs" + sObjectId);
			
			var url = "../../odata/v4/CatalogService/Pdfs" + sObjectId;
			var that = this;
			fetch(url)
			.then(response => response.json())
			.then(data => {
				
				var base64String =  "data:application/pdf;base64," + data.Content;
				
				var oPdfViewer = that.byId("PdfViewer");
				var oModel = oPdfViewer.getModel();
				oModel.setProperty("/Content", base64String);
			})
			.catch(error => 
				console.error(error)
			);
		...
		_onBindingChange : function () {
			...
			oView.getBindingContext().requestObject().then((function (oObject) {
				...
				this.addHistoryEntry({
					...
					intent: "#ManagePdfs-display&/Pdfs(" + sObjectId + ")"
				});
